...
- `.查看闭关` / `.强行出关`：查看或终止深度闭关。
- `.储物袋`：查看背包。
- `.前往 延康/大墟/宗门名`：移动到目标地点。
- `.传送 大墟`：传送阵建成后可瞬移到大墟。
- `.出售 物品名*数量`：出售材料获取铜钱/灵石。
- `.喂食 赤火灵丹/灵石*数量`：喂养龙麒麟，获得短时增益。
- `.鬼市 淘宝`：夜间鬼市抽取物品。
- `.堵门 宗门名`：挑战宗门弟子获取声望。
- `.试药`：延康药师试药（高风险/高收益）。
- `.锻造 图纸名 [自定义名]`：延康锻造（需要材料）。
- `.捐赠 项目 [铜钱/灵石] 数量`：参与全服改革进度。
- `.交易 土伯 / .偷渡 / .飘荡`：幽都复活与飘荡指令。
- `.服用 丹药名*数量`：服用丹药。
- `.炼制 配方名/目标物品*数量`：消耗材料炼制。
- `.图鉴 类别`：查看物品图鉴。
- `.宗门 查看/加入/信息/任务/签到/兑换`：宗门系统。
- `.道门 解题 [答案]`：道门入门考核（免试资格）。
- `.任务`：领取任务奖励（有冷却）。
- `.榜单 灵石/修为`：查看排行榜。
